#!/usr/bin/env python3
"""
Second parallel processor - handles VUS and other files while main process handles PLP/P
"""
...
log_file = Path(f"/tmp/clinvar_oncokb_parallel2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    # Process VUS only (it's the largest file at ~5GB)
    priority_order = ['VUS.tsv']
    
    logger.info(f"Starting PARALLEL OncoKB processor #2")
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"PARALLEL PROCESSOR #2 COMPLETE")
    logger.info(f"Total time: {total_duration/3600:.1f} hours")
    logger.info(f"Output directory: {oncokb_dir}")

if __name__ == "__main__":
    print(f"Starting parallel OncoKB processor #2")
    print(f"This will process: VUS, C, BLB, B, Other")
    print(f"While main process handles: PLP, P")
    print(f"{'='*50}\n")
