#!/usr/bin/env python3
"""
Process ClinVar files and query OncoKB API for each variant
...
BATCH_SIZE = 2000  # Maximum batch size to minimize API calls
...
def main():
    """
    Process all ClinVar significance files
    """
    ...
    # Process each ClinVar file
    for clinvar_file in clinvar_dir.glob("*.tsv"):
        output_file = oncokb_dir / clinvar_file.name
        
        # Skip if output already exists (for resuming)
        if output_file.exists():
            logger.info(f"Skipping {clinvar_file.name} - output already exists")
            continue
            
        process_clinvar_file(clinvar_file, output_file)
        
    logger.info("Completed processing all ClinVar files")
